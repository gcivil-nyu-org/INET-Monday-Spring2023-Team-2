...
class EmailVerificationView(CreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user is not None and PasswordResetTokenGenerator().check_token(user, token):
            ...
            messages.warning(request, 'The confirmation link was invalid or has expired.')
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            verification_url = request.build_absolute_uri(reverse('email_verification', kwargs={'uidb64': uidb64, 'token': token}))
